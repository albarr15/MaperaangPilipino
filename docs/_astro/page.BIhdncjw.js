...
function p() {
  for (const e of ["touchstart", "mousedown"])
    document.addEventListener(
      e,
      (t) => {
        i(t.target, "tap") && c(t.target.href, { ignoreSlowConnection: !0 });
      },
      { passive: !0 },
    );
}
...
function y() {
  ...
  u(() => {
    for (const t of document.getElementsByTagName("a"))
      s.has(t) || (i(t, "viewport") && (s.add(t), (e ??= L()), e.observe(t)));
  });
}
function L() {
  const e = new WeakMap();
...
function c(e, t) {
  ...
  if (E(e, o))
    if (
      (d.add(e), document.createElement("link").relList?.supports?.("prefetch"))
    ) {
